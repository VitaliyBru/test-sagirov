...
class Success extends PureComponent {
  ...
  render() {
    ...
    return (
      <div className={`success ${this.state.openPopup}`} onClick={this._closePopup}>
        <div className={`success__content`}>
          <svg className={`success__icon`} width="186" height="186" viewBox="0 0 186 186" fill="none"
          ...
          <b className={`success__text`}>{children}</b>
        </div>
      </div>
    );
  }
  ...
  componentDidMount() {
    ...
    setTimeout(this._openPopup, 100);
    this.idTimer = setTimeout(toggleSineUpScreen, 3000);
  }

  _openPopup() {
    this.setState({openPopup: `success--open`});
  }

  _closePopup() {
    clearTimeout(this.idTimer);
    this.props.toggleSineUpScreen();
  }
}
